...
PRFSimpleProperty::PRFSimpleProperty (TiXmlElement *elem) : PRFProperty(elem)
{
    ...
    if (isBoolean()) {
	///\todo look up boolean any type mapping dataType->value <<= ossieSupport::strings_to_boolean(value);
    } else if (isChar()) {
	///\todo look up char any type mapping	dataType->value <<= ossieSupport::strings_to_char(value);
    } else if (isDouble()) {
	...
	///\todo look up octet any type mapping	dataType->value <<= ossieSupport::strings_to_octet(value);
    } else if (isUShort()) {
