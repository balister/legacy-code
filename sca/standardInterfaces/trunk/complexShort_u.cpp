...
standardInterfaces_i::complexShort_u::complexShort_u(const char* _portName) 
{
    portName = _portName;
...
standardInterfaces_i::complexShort_u::complexShort_u(const char* _name, const char* _domain) : portName(_name)
{
...
CORBA::Object_ptr standardInterfaces_i::complexShort_u::getPort(const char* _portName)
{
...
complexShort::usesPort::usesPort(standardInterfaces_i::complexShort_u *_base) : base(_base)
{
}
...
{
    standardInterfaces::complexShort_ptr p = standardInterfaces::complexShort::_narrow(connection);
    if (CORBA::is_nil(p)) {
        DEBUG(1, standardInterfaces, "Attempt to connect a complexShort uses port to a port that does not provide a complexShort interface.");
        return;
    }
