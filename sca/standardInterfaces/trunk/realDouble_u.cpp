...
#include "standardinterfaces/realDouble_u.h"
#include "ossie/debug.h"

standardInterfaces_i::realDouble_u::realDouble_u(const char* _portName) 
{
    portName = _portName;
...
standardInterfaces_i::realDouble_u::realDouble_u(const char* _name, const char* _domain) : portName(_name)
{
...
CORBA::Object_ptr standardInterfaces_i::realDouble_u::getPort(const char* _portName)
{
...
void standardInterfaces_i::realDouble_u::pushPacket(const PortTypes::DoubleSequence &I)
{
...
realDouble::usesPort::usesPort(standardInterfaces_i::realDouble_u *_base) : base(_base)
{
}
...
void realDouble::usesPort::connectPort(CORBA::Object_ptr connection, const char* connectionID)
{
    ...
    realDouble::ConnectionInfo c(p, connectionID);
    base->dest_ports.push_back(c);
	
}

void realDouble::usesPort::disconnectPort(const char* connectionID)
{
    omni_mutex_lock l(base->port_mutex);
    for (unsigned int i = 0; i < base->dest_ports.size(); ++i) {
	if (strcmp(base->dest_ports[i].getID(), connectionID) == 0) {
